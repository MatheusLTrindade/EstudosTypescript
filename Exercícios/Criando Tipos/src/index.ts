...
function addSatellite(name: string, planet: Planet) {
    ...
    alert(`O satélite ${name} foi aadicionado ao planeta ${planet.name} com succeso.`)
}

function removeSatellite(name: string, planet: Planet) {
    planet.satellites = planet.satellites.filter(satellite => satellite !== name)

    alert(`O satélite ${name} foi removido do planeta ${planet.name} com succeso.`)
}
...
function promptValidPlanet(callback: (planet: Planet) => void) {
    ...
    if (planet) callback(planet)
    else alert('Planeta não localizado!\nRetornando ao menu...')
}
...
function firstMenuOption() {
    ...
    const existPlanet = findPlanet(name)

    if (existPlanet) {
        alert(`O Planeta ${name} já está registrado.\nRetornando ao painel principal...`)
    } else {
        ...
        if (confirmation) addPlanet(name, [coordinateA, coordinateB, coordinateC, coordinateD], situation)
    }
}
