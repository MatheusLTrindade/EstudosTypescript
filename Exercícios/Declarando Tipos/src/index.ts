...
function sendMission(spaceship:{ name:string, crewLimit:number, crew:string[], inMission:boolean}) {
    ...
    if (spaceship.inMission) {
        ...
        if (difference > 1) alert(`${spaceship.name} não pode ser enviada para missão.\nFaltando ${difference} tripulantes para a capacidade mínima.`)
        else alert(`${spaceship.name} não pode ser enviada para missão.\nFalta ${difference} tripulante para a capacidade mínima.`)
    } else {
...
function firstMenuOption() {
    ...
    if (confirmation) addSpaceship(name, pilot, crewLimit)
    else alert(`Registro da ${name} cancelado!`)
}
...
function secondMenuOption() {
    ...
    if (spaceship) {
        ...
        if (confirmation) addCrewMember(member, spaceship)
        else alert(`Inclusão de ${member} cancelada!`)     
    } else {
...
function thirdMenuOption() {
    ...
    if (spaceship) {
        ...
        if (confirmation) sendMission(spaceship)
        else alert(`Envio da ${spaceshipName} para missão cancelado!`)
    } else {
        alert(`Não há registro da ${spaceshipName}.\nMissão cancelada!`)
    }
}
